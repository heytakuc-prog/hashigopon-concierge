...
function selectRecommendations(alternate = false) {
  ...
  // 「別の3軒」は直前の候補を避ける。営業中だけでは候補が足りない場合は、
  // 営業時間外の相性候補も明示付きで補完する。
  if (alternate && ranked.filter(item => !state.currentPickIds.includes(item.shop.id)).length < 3) {
    ranked = allRanked;
  }
...
function recommendType(shop, status) {
  ...
  if (!status.open && shop.hoursStatus === "needs_confirmation") return "営業時間は要確認";
  if (!status.open) return "営業時の相性候補";
...
function recommendReason(shop, status, tags) {
  ...
  if (!status.open && shop.hoursStatus !== "needs_confirmation") parts.push("現在は営業時間外なので、営業日時は確認が必要");
